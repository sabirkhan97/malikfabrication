...
function SparkBurst({ x, y, onDone }: any) {
...
export default function WeldingSpecializations() {
    ...
    const handleTap = (index: any, e: any) => {
        const rect = e.currentTarget.getBoundingClientRect();
        const id = Date.now() + Math.random();
        setSparks((s: any) => [...s, { id, x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 }] as any);
        setFlashing(index);
    ...
    const removeSpark = (id: any) => setSparks((s) => s.filter((sp: any) => sp.id !== id));
